...
std::map<keys, std::string> parseCommandLine(int argc, char *argv[])
{
  std::map<keys, std::string> map;
    try {
        ...
        TCLAP::SwitchArg daemonSwitch("", "daemon", "Run in daemon mode (forks to run as a daemon)", cmd, false);
        TCLAP::UnlabeledValueArg<std::string> confFileArg("configFile", "The name of the configuration file. Defaults to cs3377dirmond.conf", true, "cs3377dirmond.conf", "config  filename", false);
